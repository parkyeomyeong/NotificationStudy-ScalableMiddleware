...
```java
// OFFSET 방식 대신 커서 사용
cursor = lastEnqueuedEntity.getCreatedAt();
repo.findTop100ByStatusAndCreatedAtAfterOrderByCreatedAtAsc(PENDING, cursor);
```

OFFSET 방식은 중간에 새 레코드가 삽입되면 skip/중복이 발생가능! createdAt 커서를 이용하면 이 문제를 회피할 수 있다고 판단
...
**① 인메모리 큐 → 프로세스 재시작 시 일시 지연 가능**

큐에 들어간 ID는 JVM 메모리에만 존재합니다. 재시작 시 QUEUED 상태 알림이 유실될 수 있지만, DB 폴링 스케줄러가 다음 사이클에 재적재하므로 실질적인 데이터 유실은 없습니다. 다만 그 사이 지연은 발생합니다. 운영 수준의 내구성이 필요하다면 Kafka, RabbitMQ 같은 외부 브로커가 필요할 것으로 판단됩니다.
...
스케줄러 중복 실행 방지를 JVM 내 AtomicBoolean으로 처리했습니다. 인스턴스가 2개 이상으로 수평 확장되면 동일 레코드를 여러 인스턴스가 동시에 처리할 수 있으며 이경우를 위한 다른 방법 (MQ를 kafka같은 솔루션 사용) 등으로 처리해볼까 합니다.
...
**HTTP 성공과 비즈니스 성공을 구분한 재시도 전략**

HTTP 200이 곧 발송 성공이 아닙니다. 응답 body의 status 필드로 SUCCESS / FAILURE(논리 실패) / ERROR(물리 실패)를 구분해 재시도 여부를 결정합니다. 수신자 차단이나 잘못된 형식처럼 재시도해도 결과가 같은 케이스는 즉시 PERMANENT_FAILED로 처리합니다.
...
LIMIT/OFFSET 방식은 폴링 사이 새 레코드가 삽입되면 레코드를 건너뛸 수 있습니다. createdAt 커서를 이용해 마지막으로 처리한 시점 이후만 조회하는 방식으로 이 문제를 회피했습니다.
