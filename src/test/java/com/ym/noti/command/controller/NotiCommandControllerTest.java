...
public class NotiCommandControllerTest {
  ...
  // @Test
  @DisplayName("[Unit Test] senderService.send(...)가 호출되면 SendResult.SUCCESS를 리턴하도록 설정 (성공 시뮬레이션)")
  void notiRequestForSuccess() throws Exception {
    given(notiSenderService.send(any())).willReturn(SendResult.SUCCESS);
  ...
  // @Test
  @DisplayName("[Unit Test] senderService.send(...)가 호출되면 SendResult.ERROR를 리턴하도록 설정 (실패 시뮬레이션)")
  void notiRequestForFailure() throws Exception {
    given(notiSenderService.send(any())).willReturn(SendResult.ERROR);
